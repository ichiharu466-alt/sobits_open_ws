...
| YOLO Human | `yolo_ros` + `yoloe-26s-seg.pt` | 人物の2次元検出 |
| YOLO Object | `yolo_ros` + `best.pt` | Bring Me対象物の2次元検出 |
...
ros2 launch sobits_open_mine bring_me_system.launch.py \
  ...
  skip_profile:=none
```
※launchファイルによる起動を行いますが現時点で一斉に起動すると起動できないものが出てしまい不具合が出るため今回は使用しません

起動対象は`config/launch_arg.yaml`で切り替えます。現在は`yolo_human`と`yolo_object`が有効、`bring_me_node`と`yoloe_visual_prompt_node`が無効です。Launchファイルだけでタスク本体まで開始する場合は、`bring_me_node.enabled`を`true`にしてください。

## 人検出用・物体検出用YOLO

現在は同じRGB画像を入力とする2つの`yolo_ros`ノードを起動し、人検出と物体検出を分離しています。

| 用途 | ノード名 | モデル | 検出結果 | 描画画像 |
|---|---|---|---|---|
| 人検出 | `human_yolo_node` | `yoloe-26s-seg.pt` | `/human_yolo_node/object_boxes` | `/human_yolo_node/detected_image` |
| 物体検出 | `object_yolo_node` | `best.pt` | `/object_yolo_node/object_boxes` | `/object_yolo_node/detected_image` |

`BringMeNode`は`config/robot.yaml`から2つの購読先を読み込み、人検出結果を`latest_human_detections`、物体検出結果を`latest_detections`へ保存します。`turn_to_human()`は前者、`detect_target_object()`は後者を使用します。

```yaml
detection_topic: /object_yolo_node/object_boxes
human_detection_topic: /human_yolo_node/object_boxes
```

`best.pt`のクラスは次の4種類です。
...
`task_params.yaml`の各物体にある`yolo_labels`には、モデルが出力するクラス名を含める必要があります。
...
一覧から人検出は`/human_yolo_node/detected_image`、物体検出は`/object_yolo_node/detected_image`を選択します。YOLOE Visual Promptノードの実装と設定は互換用に残していますが、現在の起動構成では使用しません。
...
| `/head_rgbd_sensor/rgb/image_rect_color` | `sensor_msgs/msg/Image` | YOLO入力 | 人検出用・物体検出用YOLOの共通入力 |
| `/human_yolo_node/object_boxes` | `vision_msgs/msg/Detection2DArray` | Subscribe | 人検出結果 |
| `/object_yolo_node/object_boxes` | `vision_msgs/msg/Detection2DArray` | Subscribe | 物体検出結果 |
| `/human_yolo_node/detected_image` | `sensor_msgs/msg/Image` | Publish | 人検出の描画画像 |
| `/object_yolo_node/detected_image` | `sensor_msgs/msg/Image` | Publish | 物体検出の描画画像 |
...
Topic名は主に`config/robot.yaml`で変更できますが、今回は使用しませんいつかどのrobotでもできるようにしたいなと思ってます汗
...
| `config/launch_arg.yaml` | 人検出用・物体検出用YOLOを含む外部Launchと各ノードの有効・無効 |
| `config/skip_state.yaml` | テスト目的別にスキップする状態のプロファイル |
| `config/yoloe_visual_prompt.yaml` | 現在は未使用。Visual Promptへ切り替える場合の設定 |

※amlファイルはlaunchファイルの不具合のためlaunch_arg, robot, skip_state.yamlは使用できませんし、robot_yamlに関しては今後違うロボットで使用することができないかなと思っておいたものです現時点有用なものではありません
...
```bash
ros2 topic echo /human_yolo_node/object_boxes
ros2 topic echo /object_yolo_node/object_boxes
ros2 topic hz /human_yolo_node/detected_image
ros2 topic hz /object_yolo_node/detected_image
