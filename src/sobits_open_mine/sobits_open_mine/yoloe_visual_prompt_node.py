...
class YoloeVisualPromptNode(Node):
    ...
    def __init__(self):
        ...
        # 入出力topic
        self.declare_parameter("image_topic", "/head_rgbd_sensor/rgb/image_raw")
    ...
    def _setup_visual_prompt(self) -> None:
        prompt_path = self._resolve_prompt_path(self.prompt_image_path)
        self.prompt_bbox = self._resolve_prompt_bbox(prompt_path)
        x1, y1, x2, y2 = self.prompt_bbox

        if x2 <= x1 or y2 <= y1:
            raise ValueError(
                f"Invalid prompt_bbox: {self.prompt_bbox}. "
        ...
        self.get_logger().info("Creating visual prompt embedding...")
        ...
        visual_embedding = self.model.model.pe.clone()

        class_names = list(self.text_prompt_classes) + [self.target_class_name]
    ...
    def _resolve_prompt_bbox(self, prompt_path: Path) -> list[float]:
        """Use an explicit valid bbox, otherwise load the selector sidecar."""
        if self._is_valid_bbox(self.prompt_bbox):
            return self.prompt_bbox

        bbox_path = (
            self._resolve_prompt_path(self.prompt_bbox_file)
            if self.prompt_bbox_file
